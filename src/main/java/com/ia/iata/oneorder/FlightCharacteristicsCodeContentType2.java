//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, v2.3.0 
// See <a href="https://javaee.github.io/jaxb-v2/">https://javaee.github.io/jaxb-v2/</a> 
// Any modifications to this file will be lost upon recompilation of the source schema. 
// Generated on: 2025.09.09 at 04:05:01 AM GST 
//


package com.ia.iata.oneorder;

import javax.xml.bind.annotation.XmlEnum;
import javax.xml.bind.annotation.XmlEnumValue;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>Java class for FlightCharacteristicsCodeContentType.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * <p>
 * <pre>
 * &lt;simpleType name="FlightCharacteristicsCodeContentType"&gt;
 *   &lt;restriction base="{http://www.w3.org/2001/XMLSchema}string"&gt;
 *     &lt;enumeration value="AirportChange"/&gt;
 *     &lt;enumeration value="Direct"/&gt;
 *     &lt;enumeration value="NonStop"/&gt;
 *     &lt;enumeration value="OvernightStop"/&gt;
 *     &lt;enumeration value="RedEye"/&gt;
 *   &lt;/restriction&gt;
 * &lt;/simpleType&gt;
 * </pre>
 * 
 */
@XmlType(name = "FlightCharacteristicsCodeContentType", namespace = "http://www.iata.org/IATA/2015/EASD/00/IATA_OffersAndOrdersCommonTypesFullyOptional")
@XmlEnum
public enum FlightCharacteristicsCodeContentType2 {


    /**
     * Airport Change
     * 
     */
    @XmlEnumValue("AirportChange")
    AIRPORT_CHANGE("AirportChange"),

    /**
     * Direct
     * 
     */
    @XmlEnumValue("Direct")
    DIRECT("Direct"),

    /**
     * Non Stop
     * 
     */
    @XmlEnumValue("NonStop")
    NON_STOP("NonStop"),

    /**
     * Overnight Stop
     * 
     */
    @XmlEnumValue("OvernightStop")
    OVERNIGHT_STOP("OvernightStop"),

    /**
     * Red eye
     * 
     */
    @XmlEnumValue("RedEye")
    RED_EYE("RedEye");
    private final String value;

    FlightCharacteristicsCodeContentType2(String v) {
        value = v;
    }

    public String value() {
        return value;
    }

    public static FlightCharacteristicsCodeContentType2 fromValue(String v) {
        for (FlightCharacteristicsCodeContentType2 c: FlightCharacteristicsCodeContentType2 .values()) {
            if (c.value.equals(v)) {
                return c;
            }
        }
        throw new IllegalArgumentException(v);
    }

}
